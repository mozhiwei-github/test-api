...
@allure.feature('接口 - 驱动搜索 - /driversearch/api/search')
class TestFile:

    def setup_class(self):
        self.url = "/driversearch/api/search"
    ...
    # 正常参数,keyword="宏基显卡"
    @allure.story('正常参数,keyword="宏基显卡"')
    def test01(self, case_config):
        ...
        try:
            ...
            assert '显卡' == response.json()['items'][0]['device_class']
    ...
    def test03(self, case_config):
        ...
        try:
            assert 200 == response.status_code
            assert 4006001 == response.json()['resp_common']['ret']
...
# 正常参数,keyword=华硕
    ...
    def test07(self, case_config):
        ...
        try:
            ...
            assert 0 == response.json()['resp_common']['ret']
            assert 'ok' == response.json()['resp_common']['msg']
            assert '华硕' in response.json()['items'][0]['driver_device_name']
        except Exception as e:
            logger.exception(e)
            raise Exception("接口返回不符合预期")

# 正常参数,keyword=指纹
    @allure.story('正常参数,keyword=指纹')
    def test08(self, case_config):
        ...
        try:
            ...
            assert 0 == response.json()['resp_common']['ret']
            assert 'ok' == response.json()['resp_common']['msg']
            assert '指纹' in response.json()['items'][0]['driver_device_name']
        except Exception as e:
            logger.exception(e)
            raise Exception("接口返回不符合预期")
